...
async function insertSchedule(schedule) {
  ...
  let painel = await new Edoc2024(schedule).save();
...
async function updateSchedule(id, schedule) {
  ...
  return await Edoc2024.findOneAndUpdate({ _id: id }, schedule);
...
async function deleteSchedule(id) {
  ...
  return await Edoc2024.findOneAndRemove({
    _id: id
  });
...
async function unsubscribeEdoc2024(workId, userId) {
  ...
  return Edoc2024.findOneAndUpdate({
    _id: workId
  }, {
    $pull: {
      subscribers: {
...
async function getSchedulesDates(mySchedules) {
  ...
  for (let index = 0; index < mySchedules.cursosInscritos.length; index++) {
    switch (mySchedules.cursosInscritos[index].icModalityId) {
      ...
      break;
      case 5: //painel
        await schedulesDatesCheck.push(await Edoc2024.findById(mySchedules.cursosInscritos[index].idSchedule).select('dates -_id'));
      break;
    }
  }
...
async function checkSubscribeDup(workId, userId) {
  ...
  let scheduleCompare = await Edoc2024.findById(workId);
...
async function subscribeEdoc2024(workId, userId, email) {
